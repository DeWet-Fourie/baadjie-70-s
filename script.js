...
/* =========================================================
   OPTION 7: CLOTHING RAIL PRODUCT BEHAVIOUR
   ========================================================= */
(function(){
  function setupRail(){
    const grid = document.querySelector("#product-grid");
    const drops = document.querySelector(".drops-panel");
    if (!grid || !drops || grid.dataset.railReady === "true") return;
    ...
    applyIndex();
    new MutationObserver(applyIndex).observe(grid, { childList:true, subtree:false });
  ...
  setTimeout(setupRail, 500);
  setTimeout(setupRail, 1500);
})();
