...
/* =========================================================
   PRODUCT FULLSCREEN OPEN FIX
   Reliable click-to-open for rail cards, with drag protection.
   ========================================================= */
(function(){
  let dragStartX = 0;
  let dragStartY = 0;
  ...
  document.addEventListener("pointermove", function(e){
    ...
    if (dx > 8 || dy > 8) isDraggingRail = true;
  }, true);
  ...
  function getProductData(card){
    ...
    return {
      category: getText(card, ".product-category") || "Baadjie",
      title: getText(card, ".product-title") || "Baadjie piece",
      meta: getText(card, ".product-meta"),
      desc: getText(card, ".product-desc"),
      price: getText(card, ".price"),
      imageSrc: image ? image.getAttribute("src") : "",
      imageAlt: image ? image.getAttribute("alt") || getText(card, ".product-title") : "",
      bg: imageWrap ? getComputedStyle(imageWrap).getPropertyValue("--product-bg") : ""
    };
  }
      ...
      <article class="product-fullscreen-card" role="dialog" aria-modal="true">
        ...
        <div class="product-fullscreen-info">
          ...
          <div class="product-fullscreen-bottom">
            ...
            <a class="product-fullscreen-link" href="#" target="_blank" rel="noreferrer">Claim piece</a>
          </div>
        </div>
      </article>
    `;
  ...
  function openModal(card){
    ...
    const img = modal.querySelector(".product-fullscreen-image img");

    modal.querySelector(".product-fullscreen-category").textContent = data.category;
    modal.querySelector(".product-fullscreen-title").textContent = data.title;
    modal.querySelector(".product-fullscreen-meta").textContent = data.meta;
    modal.querySelector(".product-fullscreen-desc").textContent = data.desc;
    modal.querySelector(".product-fullscreen-price").textContent = data.price;
    img.src = data.imageSrc;
    img.alt = data.imageAlt;

    if (data.bg) imageBox.style.setProperty("--product-bg", data.bg);

    const claim = modal.querySelector(".product-fullscreen-link");
    const whatsappNumber = (window.BAADJIE_CONFIG && window.BAADJIE_CONFIG.whatsappNumber) || "270000000000";
    const msg = encodeURIComponent(`Hi, I want to claim this Baadjie piece: ${data.title}`);
    claim.href = `https://wa.me/${whatsappNumber}?text=${msg}`;
