...
/* =========================================================
   BAADJIE SIGNATURE MICRO-INTERACTIONS
   ========================================================= */
(function(){
  const gondola=document.querySelector('.gondola-car'); const gondolaWrap=document.querySelector('.baadjie-scroll-gondola');
  function updateGondola(){ if(!gondola||!gondolaWrap)return; const max=document.documentElement.scrollHeight-window.innerHeight; const p=max>0?window.scrollY/max:0; const track=gondolaWrap.clientHeight-gondola.clientHeight; gondola.style.transform=`translate(-50%, ${track*p}px)`; }
  updateGondola(); window.addEventListener('scroll',updateGondola,{passive:true}); window.addEventListener('resize',updateGondola);
  document.addEventListener('pointermove',function(e){ const image=e.target.closest('.product-image'); if(!image)return; const rect=image.getBoundingClientRect(); const x=((e.clientX-rect.left)/rect.width)*100; const y=((e.clientY-rect.top)/rect.height)*100; image.style.setProperty('--mx',`${x.toFixed(2)}%`); image.style.setProperty('--my',`${y.toFixed(2)}%`); },{passive:true});
  const stamps=[['INNSBRUCK','1987','FOUND AGAIN'],['CHAMONIX','1994','SKI ARCHIVE'],['DOLOMITES','1982','ONE PIECE'],['ST. MORITZ','1991','WINTER STOCK'],['AUSTRIA','1998','DROP VERIFIED'],['GENEVA','1989','OUTERWEAR']];
  let stamp=document.querySelector('.vintage-stamp'); if(!stamp){ stamp=document.createElement('div'); stamp.className='vintage-stamp'; stamp.innerHTML='<span class="stamp-place"></span><span class="stamp-year"></span><span class="stamp-small"></span>'; document.body.appendChild(stamp); }
  function showStamp(){ const s=stamps[Math.floor(Math.random()*stamps.length)]; stamp.querySelector('.stamp-place').textContent=s[0]; stamp.querySelector('.stamp-year').textContent=s[1]; stamp.querySelector('.stamp-small').textContent=s[2]; stamp.classList.remove('is-visible'); void stamp.offsetWidth; stamp.classList.add('is-visible'); clearTimeout(stamp._hideTimer); stamp._hideTimer=setTimeout(()=>stamp.classList.remove('is-visible'),1800); }
  document.addEventListener('click',function(e){ if(e.target.closest('.product-card')) setTimeout(showStamp,90); });
})();
